...
export async function fetchHistoricalStockData(
    ...
    try {
        ...
        if (!result || !result.quotes || result.quotes.length === 0) {
            console.warn(`No historical quotes found for ${ticker} in the specified range.`);
            return { quotes: [], error: `No historical quotes found for ${ticker}` };
        }

        const quotes: HistoricalQuote[] = result.quotes
            .map((q: any) => {
                if (!q || typeof q.date === 'undefined' || typeof q.close !== 'number') {
            ...
            .sort((a: HistoricalQuote, b: HistoricalQuote) => a.date.getTime() - b.date.getTime());

        console.log(`Successfully fetched ${quotes.length} historical quotes for ${ticker}.`);
        return { quotes };
    } catch (error) {
        console.error(`Failed to fetch historical stock data for ${ticker}:`, error);
...
Generate the \`summary_analysis\` array based *only* on this methodology and the provided JSON data. Ensure conciseness and adherence to the bullet point count limit.
      `,
        ...
        return {
            success: false,
            error: e instanceof Error ? e.message : 'An error occurred during data sanitization',
        };
    }
}
